...
function notifyOfEntry(cardData) {
  console.log("Notify on entry was called")
  const { currentStatus } = cardData
  if (!jiraSettings[currentStatus])
    return console.log("Status not enabled in settings")
  if (jiraSettings[currentStatus].notifyOnEntry) {
    console.log("Send message invoked")
    sendMessage(slackChannel, notifyMessage(cardData))
  }
}
