...
function findInStagnants(query, stagnantCards) {
  const queryType = isNaN(Number(query)) ? "string" : "number"
  if (queryType === "string") {
    const matches = stagnantCards.filter(card =>
      card.cardTitle.toLowerCase().includes(query.toLowerCase())
    )

    if (matches.length  === 0) return null
    else if (matches.length > 1) { return matches }
    else const cardIndex = stagnantCards.findIndex(card =>
          card.cardTitle.toLowerCase().includes(query.toLowerCase())
        )
        
  } else if (queryType === "number") {
    const cardIndex = stagnantCards.findIndex(
      card => card.cardNumber === Number(query)
    )
  }
  return cardIndex
}
