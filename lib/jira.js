...
function processWebhook({ body, stagnantCards }) {
  ...
  if (isNotifyEnabled({ status: cardData.currentStatus }).notifyOnEntry)
    sendMessage({
      channel: slackChannel,
      message: notifyMessage(cardData)
    })

  if (isNotifyEnabled({ status: cardData.currentStatus }).monitorForStagnant) {
    addToStagnants({
      cardData: cardData,
      stagnantCards: stagnantCards
    })
...
function addToStagnants({ cardData, stagnantCards }) {
  ...
  console.log(`[${cardData.cardTitle}] added to stagnant cards.`)
  if (!!jiraSettings[currentStatus].monitorForStagnant) {
    const timeStamp = Date.now()
    const nextAlert = timeStamp + 30000
    console.log("now: " + timeStamp)
    console.log("alert: " + nextAlert)
    stagnantCards.push({
      ...cardData,
      alertCount: 1,
    ...
    console.log(`[${cardData.cardTitle}] next alert time is: ${nextAlert}`)
  }
}
...
function checkforStagnants(arr) {
  arr.forEach(card => {
    if (isPast(card.nextAlertTime)) {
      console.log(`[${card.cardTitle}] is past alert time`)
      card.alertCount++
      ...
      console.log(`[${card.cardTitle}] should send ${card.alertCount} alert`)
      sendMessage({
        channel: slackChannel,
