...
const { findPullRequest, getPrNumberFromUrl } = require("./github")
const {
  designer,
...
async function processWebhook({ body, stagnantCards }) {
  ...
  console.log(`STRIPPED NUMBER: ${body.issue.key.substr(3)}`)
  const findPr = await findPullRequest(body.issue.key.substr(3))
  const cardData = {
    ...
    prNumber: findPr.legnth > 0 ? getPrNumberFromUrl(findPr[0]) : null,
    prUrl: findPr.length > 0 ? findPr[0] : null,
